...
public class BankDriver {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		lowBalances = sequentialCounting(custList,0,custList.size()); // sequential run
		...
		// *******************************************************************************
		// --Parallel Run-----------------------------------------------------------------
		// Create all necessary setup variables
		int thread_cnt = 4;
		Runnable r[] = new Runnable[thread_cnt];
		Thread threads[] = new Thread[thread_cnt];
		int[] count = new int[thread_cnt];
		int chunk = custList.size()/thread_cnt;
		
		//Start the clock, and begin making threads
		startTime = System.currentTimeMillis();
		for (int x = 0; x < thread_cnt; x++) {
			r[x] = new ParallelReader(chunk*x, chunk * (x+1), custList, count, x);
		...
		System.out.println("Number of accounts with less than $1000 is: " + total_count);
		System.out.println("Parallel run took: " + estimatedTime + " (msecs)");
		// --End-of-Parallel-Run----------------------------------------------------------

	}// end of main

	static int sequentialCounting(ArrayList<Customer> myList, int start_pos, int end_pos) {
		int count = 0;
